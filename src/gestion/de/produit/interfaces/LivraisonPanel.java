...
public class LivraisonPanel extends javax.swing.JPanel {
    ...
    private JButton boutonAjouterLivraison;
    ...
    private JDateChooser datePaiementChooserLivraisons;
    ...
    private List<DetailLivraisonData> detailLivraisonList = new ArrayList<>();
    private JTextArea livraisonDetailsTextArea;
    private JTextArea detailsLivraisonTextArea;
    ...
    public LivraisonPanel() {
        ...
        tableauLivraisons.getSelectionModel().addListSelectionListener(e -> updateDetailsDisplay());
    }
    ...
    private void initComponents() {
        ...
        boutonAjouterLivraison = createModernButton("Ajouter Livraisons", new Color(100, 150, 220));
        boutonAjouterLivraison.addActionListener(e -> showSaisiePanel());
        panneauMenuLivraisons.add(boutonAjouterLivraison);
        ...
        boutonAjouterDetailsLivraison.addActionListener(e -> addDetailLivraison());
        ...
        boutonSupprimerDetailsLivraisons.addActionListener(e -> removeSelectedDetail());
        ...
        livraisonDetailsTextArea = new JTextArea();
        livraisonDetailsTextArea.setFont(new Font("Monospaced", Font.PLAIN, 12));
        livraisonDetailsTextArea.setEditable(false);
        detailsLivraisonTextArea = new JTextArea();
        detailsLivraisonTextArea.setFont(new Font("Monospaced", Font.PLAIN, 12));
        detailsLivraisonTextArea.setEditable(false);
        detailsSplitPane.setTopComponent(new JScrollPane(livraisonDetailsTextArea));
        detailsSplitPane.setBottomComponent(new JScrollPane(detailsLivraisonTextArea));
        ...
        panneauTableauLivraisons.add(splitPane, BorderLayout.CENTER);
    ...
    private void setupLivraisonsTabs() {
        ...
        gbcBase.gridwidth = 4;
        gbcBase.weighty = 1.0;
        gbcBase.fill = GridBagConstraints.VERTICAL;
        ...
        gbcPaiement.gridx = 1;
        gbcPaiement.weightx = 0.7;
        datePaiementChooserLivraisons = new JDateChooser();
        configureComponent(datePaiementChooserLivraisons, fieldDim);
        panneauPaiementLivraison.add(datePaiementChooserLivraisons, gbcPaiement);
        ...
        gbcPaiement.fill = GridBagConstraints.VERTICAL;
        ...
        panneauDetailsLivraison = new JPanel(new BorderLayout());
        ...
        detailsInputPanel.add(champPrixUnitaire, gbcDetails);

        gbcDetails.gridx = 0;
        gbcDetails.gridy = 6;
        gbcDetails.weightx = 0.2;
        JLabel labelPrixTotal = createLabel("Prix Total:", labelFont, labelColor);
        detailsInputPanel.add(labelPrixTotal, gbcDetails);
        gbcDetails.gridx = 1;
        gbcDetails.weightx = 0.8;
        ...
        detailsInputPanel.add(champPrixTotal, gbcDetails);
        ...
        detailsInputPanel.add(Box.createVerticalGlue(), gbcDetails);

        panneauDetailsLivraison.add(detailsInputPanel, BorderLayout.NORTH);

        String[] detailColumns = {"Produit", "Num Lot", "Quantité", "Date Exp.", "Observation", "Prix Unit.", "Prix Total"};
        ...
        setColumnWidths(detailTable, new int[]{150, 80, 80, 100, 150, 90, 100});
        panneauDetailsLivraison.add(new JScrollPane(detailTable), BorderLayout.CENTER);
    ...
    private void supprimerLivraisonSelectionnee() {
        ...
        try {
            ...
            try {
                if (stmtDeleteLivraison != null) {
                    stmtDeleteLivraison.close();
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
    ...
    private void checkFormCompletionForSave() {
        ...
        boolean paiementComplete = datePaiementChooserLivraisons.getDate() != null
                && !champPrixHT.getText().trim().isEmpty()
                && !champPrixTVA.getText().trim().isEmpty()
                && !champPrixTTC.getText().trim().isEmpty();

        boolean detailsComplete = !detailLivraisonList.isEmpty();
    ...
    private void saveAllData() {
        ...
        if (!boutonSauvegarderLivraison.isEnabled()) {
            ...
            if (comboClient.getSelectedIndex() <= 0 || dateLivraisonChooser.getDate() == null) {
                tabbedPaneLivraisons.setSelectedIndex(0);
            } else if (datePaiementChooserLivraisons.getDate() == null || champPrixHT.getText().trim().isEmpty()) {
        ...
        try {
            ...
            stmtLivraison.setDate(8, new java.sql.Date(datePaiementChooserLivraisons.getDate().getTime()));
            ...
            String queryDetailLivraison = "INSERT INTO DetailLivraison (NumLot, QuantiteLivree, DateExpiration, Observation, PrixUnitaire, PrixTotal, idProduit, idLivraison) "
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            stmtDetailLivraison = conn.prepareStatement(queryDetailLivraison);

            for (DetailLivraisonData detail : detailLivraisonList) {
                stmtDetailLivraison.setString(1, detail.numLot);
                stmtDetailLivraison.setFloat(2, detail.quantite);
                stmtDetailLivraison.setDate(3, new java.sql.Date(detail.dateExpiration.getTime()));
                stmtDetailLivraison.setString(4, detail.observation.isEmpty() ? null : detail.observation);
                stmtDetailLivraison.setFloat(5, detail.prixUnitaire);
                stmtDetailLivraison.setFloat(6, detail.prixTotal);
                HashMap<String, Integer> produitMap = (HashMap<String, Integer>) comboProduit.getClientProperty("produitMap");
                Integer idProduit = produitMap.get(detail.produit);
                stmtDetailLivraison.setInt(7, idProduit);
                stmtDetailLivraison.setInt(8, idLivraison);
                stmtDetailLivraison.addBatch();
            }
            stmtDetailLivraison.executeBatch();
            ...
            try {
                if (stmtDetailLivraison != null) {
                    stmtDetailLivraison.close();
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
    ...
    private void afficherDetailsPourImpression() {
        ...
        PreparedStatement stmt = null;
        ResultSet rs = null;
        StringBuilder livraisonText = new StringBuilder();
        ...
        try {
            ...
            boolean firstLivraison = true;
            boolean firstDetail = true;
            while (rs.next()) {
                if (firstLivraison) {
                    livraisonText.append("--- BON DE LIVRAISON ---\n");
                    livraisonText.append("ID Livraison: ").append(rs.getInt("idLivraison")).append("\n");
                    livraisonText.append("Date Livraison: ").append(rs.getTimestamp("DateLivraison") != null ? dateFormat.format(rs.getTimestamp("DateLivraison")) : "N/A").append("\n");
                    livraisonText.append("Numéro Bon: ").append(rs.getInt("NumBonLivraison")).append("\n");
                    livraisonText.append("Année: ").append(rs.getInt("AnneeLivraison")).append("\n\n");

                    livraisonText.append("--- Client ---\n");
                    livraisonText.append("Nom: ").append(rs.getString("ClientNom")).append("\n");
                    livraisonText.append("Adresse: ").append(rs.getString("ClientAdresse") != null ? rs.getString("ClientAdresse") : "N/A").append("\n\n");

                    livraisonText.append("--- Informations Livraison ---\n");
                    livraisonText.append("Statut: ").append(rs.getString("StatutLivraison")).append("\n");
                    livraisonText.append("Mode Transport: ").append(rs.getString("ModeTransport")).append("\n");
                    livraisonText.append("Transporteur: ").append(rs.getString("Transporteur")).append("\n");
                    livraisonText.append("Validation: ").append(rs.getBoolean("Validation") ? "Oui" : "Non").append("\n\n");

                    livraisonText.append("--- Informations Paiement ---\n");
                    livraisonText.append("Date Paiement: ").append(rs.getDate("DatePaiement") != null ? new SimpleDateFormat("dd/MM/yyyy").format(rs.getDate("DatePaiement")) : "N/A").append("\n");
                    livraisonText.append("Mode Paiement: ").append(rs.getString("ModePaiement")).append("\n");
                    livraisonText.append("Prix HT: ").append(String.format("%.2f", rs.getFloat("PrixHT"))).append("\n");
                    livraisonText.append("Prix TVA: ").append(String.format("%.2f", rs.getFloat("PrixTVA"))).append("\n");
                    livraisonText.append("Prix TTC: ").append(String.format("%.2f", rs.getFloat("PrixTTC"))).append("\n\n");
                    firstLivraison = false;
                }
            ...
            JTextArea printLivraisonTextArea = new JTextArea(livraisonText.toString());
            printLivraisonTextArea.setFont(new Font("Monospaced", Font.PLAIN, 12));
            printLivraisonTextArea.setEditable(false);
            JTextArea printDetailsTextArea = new JTextArea(detailsText.toString());
            printDetailsTextArea.setFont(new Font("Monospaced", Font.PLAIN, 12));
            printDetailsTextArea.setEditable(false);

            JPanel printPanel = new JPanel(new BorderLayout());
            printPanel.add(new JScrollPane(printLivraisonTextArea), BorderLayout.NORTH);
            printPanel.add(new JScrollPane(printDetailsTextArea), BorderLayout.CENTER);

            JOptionPane.showMessageDialog(this, printPanel, "Bon de Livraison - ID: " + idLivraison, JOptionPane.INFORMATION_MESSAGE);
    ...
    private void updateDetailsDisplay() {
        int selectedRow = tableauLivraisons.getSelectedRow();
        if (selectedRow == -1) {
            livraisonDetailsTextArea.setText("");
            detailsLivraisonTextArea.setText("");
            return;
        }
        ...
        try {
            ...
            while (rs.next()) {
                if (firstLivraison) {
                    livraisonText.append("ID Livraison: ").append(rs.getInt("idLivraison")).append("\n");
                    livraisonText.append("Client: ").append(rs.getString("ClientNom")).append("\n");
                    livraisonText.append("Adresse Client: ").append(rs.getString("ClientAdresse") != null ? rs.getString("ClientAdresse") : "N/A").append("\n");
                    livraisonText.append("Date Livraison: ").append(rs.getDate("DateLivraison") != null ? dateFormat.format(rs.getDate("DateLivraison")) : "N/A").append("\n");
                    livraisonText.append("Statut: ").append(rs.getString("StatutLivraison")).append("\n");
                    livraisonText.append("Mode Transport: ").append(rs.getString("ModeTransport")).append("\n");
                    livraisonText.append("Transporteur: ").append(rs.getString("Transporteur")).append("\n");
                    livraisonText.append("Validation: ").append(rs.getBoolean("Validation") ? "Oui" : "Non").append("\n");
                    livraisonText.append("Année: ").append(rs.getInt("AnneeLivraison")).append("\n");
                    livraisonText.append("Numéro Bon: ").append(rs.getInt("NumBonLivraison")).append("\n");
                    livraisonText.append("Date Paiement: ").append(rs.getDate("DatePaiement") != null ? dateFormat.format(rs.getDate("DatePaiement")) : "N/A").append("\n");
                    livraisonText.append("Mode Paiement: ").append(rs.getString("ModePaiement")).append("\n");
                    livraisonText.append("Prix HT: ").append(String.format("%.2f", rs.getFloat("PrixHT"))).append("\n");
                    livraisonText.append("Prix TVA: ").append(String.format("%.2f", rs.getFloat("PrixTVA"))).append("\n");
                    livraisonText.append("Prix TTC: ").append(String.format("%.2f", rs.getFloat("PrixTTC"))).append("\n");
                ...
                if (rs.getString("idDetailLivraison") != null) {
                    ...
                    detailsText.append("Numéro de Lot: ").append(rs.getString("NumLot")).append("\n");
                    detailsText.append("Quantité Livrée: ").append(rs.getFloat("QuantiteLivree")).append("\n");
                    detailsText.append("Date Expiration: ").append(rs.getDate("DateExpiration") != null ? dateFormat.format(rs.getDate("DateExpiration")) : "N/A").append("\n");
                    detailsText.append("Observation: ").append(rs.getString("Observation") != null ? rs.getString("Observation") : "").append("\n");
                    detailsText.append("Prix Unitaire: ").append(String.format("%.2f", rs.getFloat("PrixUnitaire"))).append("\n");
                    detailsText.append("Prix Total: ").append(String.format("%.2f", rs.getFloat("PrixTotal"))).append("\n");
                    detailsText.append("------------------------\n");
                } else if (firstDetail) {
                    detailsText.append("Aucun détail de produit trouvé pour cette livraison.\n");
                    detailsText.append("------------------------\n");
            ...
            livraisonDetailsTextArea.setText(livraisonText.toString());
            detailsLivraisonTextArea.setText(detailsText.toString());

        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this, "❌ Erreur lors de la mise à jour des détails: " + e.getMessage(), "Erreur Base de Données", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        } finally {
            try {
                if (rs != null) {
    ...
    private void addDetailLivraison() {
        if (comboProduit.getSelectedIndex() <= 0) {
            JOptionPane.showMessageDialog(this, "Veuillez sélectionner un produit.", "Champ Manquant", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (champNumLot.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Veuillez entrer un numéro de lot.", "Champ Manquant", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (champQuantiteLivraison.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Veuillez entrer une quantité livrée.", "Champ Manquant", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (dateExpirationChooserLivraison.getDate() == null) {
            JOptionPane.showMessageDialog(this, "Veuillez sélectionner une date d'expiration.", "Champ Manquant", JOptionPane.WARNING_MESSAGE);
            return;
        }
        try {
            float quantite = Float.parseFloat(champQuantiteLivraison.getText().trim().replace(',', '.'));
            if (quantite <= 0) {
                JOptionPane.showMessageDialog(this, "La quantité doit être supérieure à 0.", "Erreur de Données", JOptionPane.WARNING_MESSAGE);
                return;
            }
            float prixUnitaire = champPrixUnitaire.getText().trim().isEmpty() ? 0 : Float.parseFloat(champPrixUnitaire.getText().trim().replace(',', '.'));
            float prixTotal = champPrixTotal.getText().trim().isEmpty() ? (quantite * prixUnitaire) : Float.parseFloat(champPrixTotal.getText().trim().replace(',', '.'));
            if (prixTotal < 0) {
                JOptionPane.showMessageDialog(this, "Le prix total ne peut pas être négatif.", "Erreur de Données", JOptionPane.WARNING_MESSAGE);
                return;
            }

            DetailLivraisonData detail = new DetailLivraisonData(
                    (String) comboProduit.getSelectedItem(),
                    champNumLot.getText().trim(),
                    quantite,
                    dateExpirationChooserLivraison.getDate(),
                    champObservation.getText().trim(),
                    prixUnitaire,
                    prixTotal
            );
            detailLivraisonList.add(detail);
            ...
            JOptionPane.showMessageDialog(this, "Erreur de format pour la quantité, le prix unitaire ou le prix total: " + e.getMessage(), "Erreur de Format", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void updateDetailTable() {
        ...
        for (DetailLivraisonData detail : detailLivraisonList) {
            model.addRow(new Object[]{
                ...
                detail.numLot,
                detail.quantite,
                dateFormat.format(detail.dateExpiration),
                detail.observation,
                String.format("%.2f", detail.prixUnitaire),
                String.format("%.2f", detail.prixTotal)
            });
        }
    }
    ...
    private void removeSelectedDetail() {
        ...
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "Veuillez sélectionner un détail à supprimer.", "Aucune Sélection", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int confirm = JOptionPane.showConfirmDialog(this, "Voulez-vous vraiment supprimer ce détail ?", "Confirmation de Suppression", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            detailLivraisonList.remove(selectedRow);
            updateDetailTable();
            checkFormCompletionForSave();
            JOptionPane.showMessageDialog(this, "Détail supprimé avec succès.", "Succès", JOptionPane.INFORMATION_MESSAGE);
        }
    }

    private static class DetailLivraisonData {
        ...
        DetailLivraisonData(String produit, String numLot, float quantite, Date dateExpiration, String observation, float prixUnitaire, float prixTotal) {
